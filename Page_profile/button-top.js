...
window.onload = function () 
{
	...
	function dark_theme(){
		...
		else
		{
			...
			a.style.color = "black"
			TEXT = document.querySelectorAll(".dark-change-text")
			TEXT.forEach((id) => {id.style.color = "#0059b4"})
			BORD = document.querySelectorAll(".dark-change-border")
			BORD.forEach((id) => {id.style.borderColor = "#0059b4"})
			BODY.style.backgroundColor = "white"
			BACK = document.querySelectorAll(".dark-change-background-menu")
			BACK.forEach((id) => {id.style.background = "white"})
